  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-gray-300 border-t-[#044241]" />
      </div>
    );
  }
      ...
      {!requests || requests.length === 0 ? (
        ...
        <div className="grid gap-4">
          {requests.map((request) => {
            /* 🔒 CRITICAL GUARD — THIS FIXES YOUR CRASH */
            if (!request.programId) {
              return (
                ...
                >
                  <p className="font-semibold mb-1">Program Deleted</p>
                  <p>This update request belongs to a program that has been deleted.</p>
                </div>
              );
            }
